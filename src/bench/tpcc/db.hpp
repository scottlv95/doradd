...
class Database {
   public:
    std::vector<bool> tx_status;
    std::vector<uint64_t> tx_prepare_time;
    ...
    Database()
        : warehouse_table(),
          ...
          item_table(),
          history_table() {
        tx_status.reserve(NUM_TRANSACTIONS);
        tx_prepare_time.reserve(NUM_TRANSACTIONS);
        for (uint64_t i = 0; i < NUM_TRANSACTIONS; i++) {
            tx_status.push_back(false);
        }
    }
    ...
    // void newOrder(uint64_t w_id, uint64_t d_id, uint64_t c_id, uint64_t o_entry_d, uint32_t ol_cnt,
    //               std::vector<uint64_t>* i_ids, std::vector<uint64_t>* i_w_ids, std::vector<uint64_t>* i_qtys,
    //               uint64_t tx_id) {
    ...
    //     cown_ptr<District> d = district_table.get(std::make_pair(w_id, d_id)).value();
    //     cown_ptr<Customer> c = customer_table.get(std::make_tuple(w_id, d_id, c_id)).value();

    //     // Retrieve stocks
    //     cown_ptr<Stock> s_ptr[ol_cnt];
    //     for (uint32_t i = 0; i < ol_cnt; i++) {
    //         cown_ptr<Stock> s = stock_table.get(std::make_pair(i_w_ids->at(i), i_ids->at(i))).value();
    //         s_ptr[i] = s;
    //     }
    ...
    //     tx_prepare_time[tx_id] = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    ...
    //     when(w, d, c, s, i) << [=](auto _w, auto _d, auto _c, auto _s, auto _i) mutable {
    //         // Get next order id
    ...
    //             order_line_table.add(std::make_tuple(w_id, d_id, next_o_id, i), ol);
    ...
    // // Our current implementation does not support getting customer by surname (Similar to Caracal)
    // void payment(uint64_t w_id, uint64_t d_id, uint64_t c_id, uint64_t c_w_id, uint64_t c_d_id, uint64_t h_amount,
    //              uint64_t tx_id) {
    ...
    //     cown_ptr<Warehouse> w = warehouse_table.get(w_id).value();
    //     cown_ptr<District> d = district_table.get(std::make_pair(w_id, d_id)).value();
    //     cown_ptr<Customer> c = customer_table.get(std::make_tuple(c_w_id, c_d_id, c_id)).value();
