...
class Customer {
   public:
    ...
    uint64_t hash_key() const {
        return (c_w_id << 32) | (c_d_id << 16) | c_id;
    }
};
...
class Order {
   public:
    ...
    uint64_t hash_key() const {
        return (o_w_id << 32) | (o_d_id << 16) | o_id;
    }
...
class OrderLine {
   public:
    ...
    uint64_t hash_key() const {
        return (ol_w_id << 32) | (ol_d_id << 16) | ol_o_id | ol_number;
    }
};
...
class History {
   public:
    ...
    uint64_t hash_key() const {
        return (h_c_w_id << 32) | (h_c_d_id << 16) | h_c_id;
    }
};
