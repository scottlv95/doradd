...
public:
  ...
  void process_checkpoint_request(rigtorp::SPSCQueue<int>* ring) {
    ...
    // 9) Once every batch has finished, commit the checkpoint
    {
        std::lock_guard<std::mutex> lg(completion_mu);
        completion_thread = std::thread([this, snap, latch]() {
            ...
            auto batch = storage.create_batch();
            // Update the global committed snapshot
            storage.add_to_batch(batch, GLOBAL_SNAPSHOT_KEY, std::to_string(snap));
            // Persist the total‐transactions counter
  ...
  size_t try_recovery() {
    ...
    // 3) Scan all "<snap>_v<id>" entries and pick, for each id, the highest snap ≤ valid_snap
    if (index) {
        ...
        for (auto& kv : storage.scan_prefix("")) {
            ...
            uint64_t id = std::stoull(key.substr(pos + 2));
            if (snap_ver > valid_snap) continue;

            auto it = best.find(id);
            if (it == best.end() || it->second.first < snap_ver) {
                best[id] = {snap_ver, kv.second};
            }
        }
    ...
    // 5) Update current_snapshot to be beyond the recovered snapshot
    current_snapshot.store(valid_snap, std::memory_order_relaxed);
